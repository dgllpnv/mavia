...
| **DP-38** | `cortesia` é receita? Entra no total da escrituração? | **Não.** `valor_centavos = 0` obrigatório e o valor dispensado em campo próprio. Aparece na listagem, fica fora do total — o mesmo tratamento que a regra 12b dá à transferência | Achado **F-6** |
...
| **DP-40** | O operador pode rebaixar o plano no meio de um ciclo pago? | **Não.** O painel agenda para o fim do período, pelo mesmo caminho da rota do cliente, que já recusa — *"o cliente comprou aquele período inteiro"*. Furar a regra custa ao cliente os dias que ele pagou e não usará | Achado **F-8**, contra `spec-planos-e-assinatura.md` §7 |
